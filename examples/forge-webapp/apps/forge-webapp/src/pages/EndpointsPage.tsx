...
function defaultPayload(kind: string): string {
  if (kind === "computed_column") return JSON.stringify({ primary_keys: [""] }, null, 2);
  return "{}";
}
...
function kindColor(kind: string): string {
  ...
  if (kind === "computed_column") return "var(--accent-purple, #a855f7)";
...
export function EndpointsPage() {
  ...
  const [newKind, setNewKind] = useState<"action" | "streaming" | "computed_column">("action");
              ...
              {newResult?.file ? (
                ...
                <Container direction="column" gap="8px" padding="0" style={{ maxWidth: 420 }}>
                  ...
                  <RadioGroup
                    name="endpoint-kind"
                    value={newKind}
                    onChange={(v) => setNewKind(v as "action" | "streaming" | "computed_column")}
                    options={[
                      { value: "action", label: "action" },
                      { value: "streaming", label: "streaming" },
                      { value: "computed_column", label: "computed_column" },
                    ]}
                  />
                  {newResult?.error && (
