...
export function AppsPage() {
  ...
  useEffect(() => {
    ...
    function checkAll() {
      apps.forEach((app) => {
        callEndpoint<PingResult>(PING_APP_ID, { project_id: active!.id, app_name: app.name })
          .then((res) => setAppLive((prev) => ({ ...prev, [app.id]: res ?? { live: false } })))
          .catch(() => setAppLive((prev) => ({ ...prev, [app.id]: { live: false } })));
      });
    }
    checkAll();
      ...
      fastPolling.forEach((appId) => {
        ...
        callEndpoint<PingResult>(PING_APP_ID, { project_id: active!.id, app_name: app.name })
          .then((res) => {
            ...
            if (result.live) setFastPolling((s) => { const n = new Set(s); n.delete(appId); return n; });
          })
          .catch(() => {});
      });
    }, 600);
  ...
  const createApp = useMutation({
    mutationFn: () =>
      callEndpoint<{ path?: string; name?: string; port?: string; error?: string }>(
        CREATE_APP_ID,
        { project_id: active?.id ?? "", app_name: newAppName }
      ),
    onSuccess: (result) => {
      if (result?.error) {
        ...
        {active && (
          <ButtonGroup
            ...
            buttons={[{
              label: "+ New App",
              variant: "primary",
              action: { kind: "ui", handler: () => { setShowNew(true); setNewResult(null); setNewAppName(""); } },
            }]}
          />
        )}
      </div>
                ...
                <Container direction="column" gap="8px" padding="0" style={{ maxWidth: 420 }}>
                  ...
                  <ButtonGroup
                    ...
                    buttons={[
                      ...
                      { label: "Cancel", action: { kind: "ui", handler: () => { setShowNew(false); setNewResult(null); setNewAppName(""); } } },
                    ]}
                  />
                </Container>
              )}
                ...
                return (
                  <div key={app.id} className="card" style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                      ...
                      <Row label="Port" value={isLive && livePort ? String(livePort) : (app.port || "—")} mono />
                      {isLive && livePort && (
                        <Row label="URL" value={`http://localhost:${livePort}`} mono />
                      )}
                    </div>
