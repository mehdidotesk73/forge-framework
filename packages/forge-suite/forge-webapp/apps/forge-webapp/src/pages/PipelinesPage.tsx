...
export function PipelinesPage() {
  ...
  const pipelines = (pipelineData?.rows ?? []).filter(
  ...
  const { data: dagData } = useQuery({
    ...
    queryFn: () =>
      callEndpoint<DAGData>(DAG_ID, { project_id: active?.id ?? "" }),
    enabled: !!active,
  ...
  const pipelineObjectSet = createObjectSet(
    pipelines,
    PIPELINE_SCHEMA,
    "pipelines",
    "snapshot",
  );
  const runObjectSet = createObjectSet(
    runs,
    RUN_SCHEMA,
    "pipeline-runs",
    "snapshot",
      ...
      {!active ? (
        ...
        <Container direction='column' gap={20}>
              ...
              {newResult?.file ? (
                ...
                >
                  ...
                  <ButtonGroup
                    ...
                    buttons={[
                      {
                        label: createPipeline.isPending
                          ? "Creating…"
                          : "Create Pipeline",
                        variant: "primary",
                        disabled: !newName || createPipeline.isPending,
                        action: {
                          kind: "ui",
                          handler: () => createPipeline.mutate(newName),
                        },
                      },
                      {
                        label: "Cancel",
                ...
                interaction={{
                  ...
                  contextMenu: (row) => {
                    const items: ContextMenuItem[] = [
                      ...
                      {
                        label:
                          running && activePipeline === String(row.name)
                            ? "Running…"
                            : "▶ Run",
                        disabled: running,
                        action: {
                          kind: "ui",
                          handler: () => handleRun(String(row.name)),
                        },
                      },
                    ];
                    return items;
          ...
          {(logs.length > 0 || running) && (
            <Container
              title={activePipeline ? `Output — ${activePipeline}` : "Output"}
              titleSize='md'
              alignItems='center'
              variant='none'
            >
