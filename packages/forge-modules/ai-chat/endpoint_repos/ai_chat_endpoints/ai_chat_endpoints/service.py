...
def create_session(title: str, mode: str, model_id: str) -> dict:
    ...
    return {"session_id": sid, "title": title, "mode": mode, "model_id": model_id,
...
def resume_session(session_id: str) -> dict | None:
    ...
    if session is None:
        return None
    messages = sorted(ChatMessage.filter(session_id=session_id), key=lambda m: m.created_at)
    return {
...
def list_sessions() -> list[dict]:
    ...
    return sorted(
        [{"session_id": s.id, "title": s.title, "mode": s.mode,
          "model_id": s.model_id, "updated_at": s.updated_at, "created_at": s.created_at}
         for s in sessions],
...
def _build_system_prompt(session: Any, state: dict) -> str:
    ...
    if state["current_estimate"]:
        phase_notes.append(f"Current estimate on the table:\n{state['current_estimate']}")
...
def upload_context(session_id: str, content_type: str, content: str) -> dict:
    ...
    if content_type == "url":
        ...
        raise ValueError(f"Unsupported content_type {content_type!r}. Use 'text' or 'url'.")
    ...
    return {"status": "ok", "message": "Training context injected into session history."}
