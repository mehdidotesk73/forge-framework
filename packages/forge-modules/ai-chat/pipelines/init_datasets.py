"""init_datasets pipeline — creates empty backing datasets for ChatSession and ChatMessage.
...
def run(inputs, outputs) -> None:  # type: ignore[override]
    ...
    outputs.chat_sessions.write(pd.DataFrame({
    ...
    outputs.chat_messages.write(pd.DataFrame({
        "id":         pd.Series([], dtype="str"),
